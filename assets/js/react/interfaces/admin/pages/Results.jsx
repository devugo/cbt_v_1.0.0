...
import ResultSheet from '../../user/components/ResultSheet';
...
const Results = () => {
    ...
    const viewResult = useCallback((examTaken, exam, user) => {
        setExamTakenIRI(examTaken);
        setExamConcluded(exam);
        setUser(users.find(usr => usr.iri === user));

        setResultMode(prevState => !prevState);
    }, [setExamConcluded, setExamTakenIRI, setResultMode, setUser, users]);
    ...
    const getUsers = useCallback(async (page = 1, pagination = false, userGroup = null) => {
        try {
    ...
    const getExamsTaken = useCallback(async (page = 1, pagination = true, exam = null, user = null) => {
        // setFilteredexams(); setexams(); 
        setLoading({
            ...loading,
            content: true
    ...
    const renderTableData = () => {
        if (!filteredExamsTaken || !users || !exams){
            ...
            return (
                <React.Fragment key={index}>
                    <tr className="tr-shadow">
                        ...
                        <td>{exams.find(exam => exam.iri === exmTak.exam).title}</td>
                        <td>{users.find(user => user.iri === exmTak.user).username}</td>
                        ...
                        <td>
                            <Tooltip placement="top" title="View result"><BookRoundedIcon onClick={()=>viewResult(exmTak.iri, exmTak.exam, exmTak.user)} style={{cursor: "pointer", color: 'green'}} color="secondary" /></Tooltip>
                        </td>
                    </tr>
                            ...
                            <>
                                ...
                                <div className="table-responsive table-responsive-data2">
                                    <table className="table table-data2">
                                        ...
                                        <tbody>
                                            {users.length > 0 && exams.length > 0 && renderTableData()}
                                        </tbody>
                                    </table>
                                    {
